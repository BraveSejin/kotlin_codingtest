...
fun main() {
    ...
    arr2.forEach { println("${it.key} ${it.value}") }
    ...
    println("===============3================")
    val arr4 = arr3.groupBy { it.first().toUpperCase() }    // Key : 첫글자
    arr4.forEach { println(it) }


    println("===============4================")
    val arr5 = arr3.groupBy( { it.first().uppercase() }, { it.toUpperCase() } )   // Key, Value
